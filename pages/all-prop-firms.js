...
const AllPropFirms = ({ blogs }) => {
  ...
  const handleLikeToggle = async (firmId) => {
    ...
    setPropFirms((prev) =>
      prev.map((firm) =>
        firm.id === firmId
          ? { ...firm, likes: userLikedFirms.has(firmId) ? firm.likes - 1 : firm.likes + 1 }
          : firm
      )
    );
                  ...
                  >
                    ...
                    <Link href={`/prop-firms/${firm.slug}`} passHref>
                      ...
                      <div className="flex justify-between px-7">
                        <div className="w-20 h-20 mb-2 flex items-center justify-center rounded-[10px] p-1 mt-[50px]" 
                            style={{ backgroundColor: firm.brand_colour }}>
                          <Image src={firm.logo_url} alt={firm.propfirm_name} width={48} height={40} 
