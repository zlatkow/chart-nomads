...
// Remove the direct import of LoginModal
...
const IssueReportForm = () => {
  ...
  useEffect(() => {
    const fetchPropFirms = async () => {
      const { data, error } = await supabase.from("prop_firms").select("id, propfirm_name")
  ...
  const handleSubmit = async (e) => {
    ...
    // Fix: use ruleDescription instead of mistakeDescription
    const { error } = await supabase.from("banned_countries_mistake").insert([
      {
        prop_firm_id: selectedFirm,
        mistake_description: ruleDescription.trim(), // Fixed variable name
      },
    ])
  ...
  return (
    <div className="relative mt-20 bg-[#0f0f0f] border-[#EDB900] bg-[rgba(237,185,0,0.03)] border-[1px] p-10 rounded-[10px] text-white w-[80%] mx-auto mb-[150px]">
      ...
      <h2 className="text-6xl font-bold text-center mb-2 mt-5 text-[#EDB900]">
        Noticed an issue or missing information?
      </h2>
      <p className="text-center mb-5 mt-3 text-white">Let us know if we should fix anything.</p>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col space-y-5">
        ...
        <div>
          <label className="block mb-2 text-[#EDB900]">Issue Description</label>
          <textarea
            value={ruleDescription}
            onChange={(e) => setRuleDescription(e.target.value)}
            placeholder="Provide us with the information about the issue."
