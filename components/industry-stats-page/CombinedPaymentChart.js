...
const CombinedPaymentChart = ({ monthlyStats }) => {
  ...
  useEffect(() => {
    ...
    const filteredData = monthlyStats.filter((entry) => {
      let entryDate = new Date(entry.year, new Date(entry.month + " 1, 2000").getMonth()); // Convert text month to date
  ...
  useEffect(() => {
    ...
    const transformedData = filteredStats.map((entry) => {
      ...
      const monthlyAmount = parseFloat(entry.totalamount) / 1_000_000 || 0;
  ...
  const totalCumulativeAmount = chartData.length 
    ? chartData[chartData.length - 1].cumulativeAmount.toFixed(2)
    : 0;
    ...
    <div className="bg-[#0f0f0f] text-white rounded-lg border-[1px] border-[#666666] mb-[50px] pb-6">
      <div className="flex justify-between items-center mb-6 border-b-[1px] border-[#666666] p-6">
        <div>
          ...
          <div>
            <p className="text-[#666666]">Monthly & cumulative amounts paid by the industry</p>
          </div>
        </div>
...
export default CombinedPaymentChart;
